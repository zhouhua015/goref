...
import (
	subject "github.com/zhouhua015/goref/tests/pkg/sbj"
	"bytes"
...
func (ctx *Context) ParseSubject() error {
	...
	// try to get scope
	declPos := ctx.Subject.ObjDeclPos()
...
func (ctx *Context) buildSubject(identifier ast.Expr, f *ast.File, typ types.Type, obj *ast.Object) error {
	...
	case *ast.SelectorExpr:
		...
		ctx.Subject = &subject.SelectorSub{Self: t, Typ: typ, Obj: obj, Recv: typeOf(t.X)}
	case *ast.Ident:
		// *ast.Ident might be
		...
		case *ast.FuncDecl:
			...
			if d.Recv != nil {
				...
				ctx.Subject = &subject.SelectorSub{Self: e,
					Typ:  typ,
					Obj:  obj,
		...
		case *ast.Field:
			...
			if owner == nil {
				ctx.Subject = &subject.IdentSub{Self: t, Typ: typ, Obj: obj}
				break
			}
			e := &ast.SelectorExpr{X: owner, Sel: t}
			debugp("subject recv type: %v", typeOf(owner))
			ctx.Subject = &subject.SelectorSub{Self: e, Typ: typ, Obj: obj, Recv: typeOf(owner)}
		}
...
func (ctx *Context) SubjectDeclPos() (token.Position, bool) {
	...
	if _, ok := ctx.Subject.(*subject.SelectorSub); ok {
		// if decl file is in search path, give decl position as
		// a reference, too.
		subPosition := types.FileSet.Position(ctx.Subject.ObjDeclPos())
